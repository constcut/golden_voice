...
class ReportGenerator:
    ...
    def request_recognition(self, record_file_path, alias_name):
        ...
        filelink = 'https://storage.yandexcloud.net/' + \
            self._config["bucket"] + '/' + alias_name
    ...
    def statistic_value(self, sequence, type):
        sub_sequence = [x for x in sequence if (
            math.isnan(x) == False and x != 0)]
    ...
    def parse_praat_info(self, info_text):
        ...
        fields = [["duration:", "seconds"], ["Median pitch:", "Hz"], ["Mean pitch:", "Hz"],
                  ["Standard deviation:", "Hz"], [
                      "Minimum pitch:", "Hz"], ["Maximum pitch:", "Hz"],
            ...
            if name_pos != -1:
                ...
                if field_sepparator != "":
                    sep_pos = info_text.find(
                        field_sepparator, name_pos + len(field_name))
                    next_line_pos = info_text.find("\n", name_pos)
                    if sep_pos > next_line_pos:
                        field_value = "0"
                    else:
                        field_value = info_text[name_pos +
                                                len(field_name): sep_pos - 1]
                else:
                    sep_pos = info_text.find("\n", name_pos)
                    field_value = info_text[name_pos +
                                            len(field_name): sep_pos]
    ...
    def measure_report_time(self, start_moment, reshape_sequences_moment,
        ...
        print("Total on report: ", total_spent.seconds,
              "s ", total_spent.microseconds / 1000.0, " ms")
        ...
        print("Reshape sequences ", rehape_spent.seconds,
              "s ", rehape_spent.microseconds / 1000.0, " ms")
        print("Surf ", surf_spent.seconds, "s ",
              surf_spent.microseconds / 1000.0, " ms")
        print("Praat ", praat_spemt.seconds, "s ",
              praat_spemt.microseconds / 1000.0, " ms")
        print("All chunks", all_chunks_spent.seconds, "s ",
              all_chunks_spent.microseconds / 1000.0, " ms")
        print("Cross matrix ", cross_spent.seconds, "s ",
              cross_spent.microseconds / 1000.0, " ms")
        print("Dump spent, ", dump_spent.seconds, "s ",
              dump_spent.microseconds / 1000.0, " ms")
    ...
    def make_chunk_report_part(self, alt, tags, chunks):
        ...
        pitch_cut = self.make_sequence_cut(
            pitch_step, first_start, prev_word_end, pitch)
        intens_cut = self.make_sequence_cut(
            intensity_step, first_start, prev_word_end, intensity)
        ...
        if self.calc_every_stat:
            ...
            if self.use_rosa:
                # TODO fix, probably librosa broken
                f0_cut = self.make_sequence_cut(
                    f0_step, first_start, prev_word_end, f0)
                rms_cut = self.make_sequence_cut(
                    rms_step, first_start, prev_word_end, rms)
                statistics_records["pyin_pitch"] = self.get_full_statistics(
                    f0_cut)
                statistics_records["rms"] = self.get_full_statistics(rms_cut)
            if self.use_surf:
                swipe_cut = self.make_sequence_cut(
                    swipe_step, first_start, prev_word_end, swipe_pitch)
                surf_intens_cut = self.make_sequence_cut(
                    surf_intens_step, first_start, prev_word_end, surf_intensity)
                statistics_records["surf intensity"] = self.get_full_statistics(
                    surf_intens_cut)
                statistics_records["swipe_pitch"] = self.get_full_statistics(
                    swipe_cut)
        chunk_text = alt["text"]
    ...
    def make_json_report(self, req, seq_dict, time, date):
        ...
        if self.use_surf:
            full_stats["surf intensity"] = self.get_full_statistics(
                surf_intensity)
        ...
        for chunk in req['response']['chunks']:  # Refact step #2
            ...
            if channel_id != '1':
                # print("skipping")
                continue
            # We don't handle silence right yet in case for alts ATTENTION
            for alt in chunk['alternatives']:
                ...
                for word in alt['words']:  # Refact step #1
                    ...
                    pause_RMS = self.make_sequence_cut(
                        rms_step, silence_start, silence_end, rms)
                    pause_intens = self.make_sequence_cut(
                        intensity_step, silence_start, silence_end, intensity)
                    if self.use_surf:
                        pause_surf_intens = self.make_sequence_cut(
                            surf_intens_step, silence_start, silence_end, surf_intensity)
                    if self.use_rosa:
                        pause_RMS = self.get_full_statistics(pause_RMS)
                    pause_intens = self.get_full_statistics(pause_intens)
                    if self.use_surf:
                        pause_surf_intens = self.get_full_statistics(
                            pause_surf_intens)
                    single_pause = {"type": "pause",
                                    "start": silence_start, "end": silence_end}
                    ...
                    if self.include_sequences:
                        if self.use_rosa:
                            f0_cut = self.make_sequence_cut(
                                f0_step, start, end, f0)
                            rms_cut = self.make_sequence_cut(
                                rms_step, start, end, rms)
                        pitch_cut = self.make_sequence_cut(
                            pitch_step, start, end, pitch)
                        intens_cut = self.make_sequence_cut(
                            intensity_step, start, end, intensity)
                        if self.use_surf:
                            swipe_cut = self.make_sequence_cut(
                                swipe_step, start, end, swipe_pitch)
                            surf_intens_cut = self.make_sequence_cut(
                                surf_intens_step, start, end, surf_intensity)
                    if self.calc_every_stat:
                        statistics_records = {"praat_pitch": self.get_full_statistics(pitch_cut),
                                              "intensity": self.get_full_statistics(intens_cut)}
                        if self.use_surf:
                            ...
                            statistics_records["surf intensity"] = self.get_full_statistics(
                                surf_intens_cut)
                        if self.use_rosa:
                            statistics_records["rms"] = self.get_full_statistics(
                                rms_cut)
                            statistics_records["pyin_pitch"] = self.get_full_statistics(
                                f0_cut)
                    ...
                    singleWord = {
                        ...
                        # fastest fix
                        "letters_freq": 1.0 / (letters_speed + 0.00000000001)
                    }
                    if self.calc_every_stat:
                        singleWord["stats"] = statistics_records
                    if self.every_word_praat_report:
                        singleWord["info"] = self.parse_praat_info(
                            report_string)
                    if self.use_rosa:
        ...
        if self.use_cross_matrix:
            cross_stats = self.make_cross_matrix(
                all_starts, all_ends, snd, pitch_for_praat, pulses)
    ...
    def save_downloaded_and_name(self, path_user_logs, message, downloaded_file):
        record_file_path = path_user_logs + \
            '/record_' + str(message.id) + '.ogg'
        if self.verbose:
            print(record_file_path, " <- dir path")
        with open(os.path.join(record_file_path), 'wb') as new_file:
            new_file.write(downloaded_file)
        alias_name = "a" + str(message.chat.id) + "b" + \
            str(message.id) + ".ogg"
    ...
    def send_message_and_reports(self, path_user_logs, message, message_text, tags):
        ...
        if self.verbose:
            doc = open(path_user_logs + '/stt_' +
                       str(message.id) + '.json', 'rb')
            self.bot.send_document(message.chat.id, doc)
        doc = open(path_user_logs + '/full_report_' +
                   str(message.id) + '.json', 'rb')
        self.bot.send_document(message.chat.id, doc)

    def deplayed_audio_document(self, path_user_logs, message, downloaded_file):
        self.bot.reply_to(
            message, 'Анализ mp3\wav файлов может не работать с длинными записями')
        time = datetime.datetime.now().strftime('%H:%M:%S')
        date = datetime.datetime.now().strftime('%Y-%m-%d')
        if message.document != None:
            record_file_path = path_user_logs + '/audio_' + \
                str(message.id) + "_" + message.document.file_name
        else:
            record_file_path = path_user_logs + '/audio_' + \
                str(message.id) + "_" + message.audio.file_name
        ...
        wav_file = self.convert_ogg_to_wav(
            path_user_logs, new_file, str(message.id))
    ...
    def deplayed_recognition(self, path_user_logs, message, downloaded_file):
        try:
            time = datetime.datetime.now().strftime('%H:%M:%S')
            date = datetime.datetime.now().strftime('%Y-%m-%d')
            record_file_path, alias_name = self.save_downloaded_and_name(
                path_user_logs, message, downloaded_file)
            print(record_file_path)
            self.convert_wav_to_ogg(
                record_file_path, record_file_path + ".ogg")
            id = self.request_recognition(
                record_file_path + ".ogg", alias_name)
            wav_file = self.convert_ogg_to_wav(
                path_user_logs, record_file_path, str(message.id))
            seq_dict = self.extract_features(wav_file)
            if self.skip_plots == False:
                self.save_images(seq_dict)
            # here we may loose a lot of time TODO async + queue
            req = self.check_server_recognition(id)
            # print(req)
            full_string = json.dumps(req, ensure_ascii=False, indent=2)
            json_report, tags = self.make_json_report(
                req, seq_dict, time, date)
            import hashlib
            with open(record_file_path, 'rb') as file_obj:
                file_contents = file_obj.read()
                json_report["audio_md5"] = hashlib.md5(
            ...
            if self.required_cleaning:
                ...
                full_report_name = path_user_logs + \
                    '/full_report_' + str(message.id) + '.json'
    ...
    # Отделить всю отрисовку в отдельный класс
    def draw_intensity_praat(self, intensity):
    ...
    def set_handlers(self):
        ...
        def process_docs_audio(message):
            ...
            print("Doc file-name", message.document.file_name,
                  "ext", file_extenstion)
            if file_extenstion != "mp3" and file_extenstion != "wav":
                self.bot.reply_to(message, "Допускаются только .wav или .mp3")
            else:
                self.bot.reply_to(
                    message, f"Аудио обрабатывается. Момент: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

                t = threading.Timer(1.0, self.deplayed_audio_document, [
                                    path_user_logs, message, downloaded_file])
                t.start()
        ...
        def process_docs_audio(message):
            ...
            print("Doc file-name", message.audio.file_name,
                  "ext", file_extenstion)
            if file_extenstion != "mp3" and file_extenstion != "wav":
                self.bot.reply_to(message, "Допускаются только .wav или .mp3")
            else:
        ...
        def process_voice_message(message):
            try:
                path_user_logs = self._config["dir"] + \
                    '/' + str(message.chat.id)
                if not os.path.exists(path_user_logs):
                    os.makedirs(path_user_logs)
                ...
                downloaded_file = self.bot.download_file(file_info.file_path)
                self.bot.reply_to(
                    message, f"Голос обрабатывается. {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                t = threading.Timer(1.0, self.deplayed_recognition, [
                                    path_user_logs, message, downloaded_file])
                t.start()
                if self.verbose:
                    print("Audio saved")
    ...
    def extract_features(self, wav_file):
        ...
        if self.measure_time:
            ...
            print("Total ", total_spent.seconds, "s ",
                  total_spent.microseconds / 1000.0, " ms")
            print("Rosa load ", librosa_load_spent.seconds, "s ",
                  librosa_load_spent.microseconds / 1000.0, " ms")
            print("Rosa stft ", librosa_stft_spent.seconds, "s ",
                  librosa_stft_spent.microseconds / 1000.0, " ms")
            print("Rosa rest ", librosa_rest_spent.seconds, "s ",
                  librosa_rest_spent.microseconds / 1000.0, " ms")
            print("Praat ", praat_spent.seconds, "s ",
                  praat_spent.microseconds / 1000.0, " ms")
            print("Surf ", surf_spent.seconds, "s ",
                  surf_spent.microseconds / 1000.0, " ms")
        return seq_dict
    ...
    def local_recognition(self, path_user_logs, record_file_path, alias_name):
        ...
        wav_file = self.convert_ogg_to_wav(
            path_user_logs, record_file_path, "local_id")
        seq_dict = self.extract_features(wav_file)
        if self.skip_plots == False:
            # возможно надо сохранять ещё директорию, но вначале возьмём из конфига - abadoned
            self.save_images(seq_dict)
        ...
        full_report_generated = datetime.datetime.now()
        self.save_json_products(
            path_user_logs, json_report, full_string, "local_id")
        if self.measure_time:
            ...
            total_spent = full_report_generated - start_moment
            print("Spent on send: ", spent_on_send.seconds, "s ",
                  spent_on_send.microseconds / 1000.0, " ms")
            print("Spent on imaged: ", spent_on_imaged.seconds,
                  "s ", spent_on_imaged.microseconds / 1000.0, " ms")
            print("Spent on received: ", spent_on_received.seconds,
                  "s ", spent_on_received.microseconds / 1000.0, " ms")
            print("Spent on report: ", spent_on_report.seconds,
                  "s ", spent_on_report.microseconds / 1000.0, " ms")
            print("Total spent ", total_spent.seconds, "s ",
                  total_spent.microseconds / 1000.0, " ms")
        print("Done!")
...
def reports_to_csv(r):
    with open(r._config['to-csv'] + '/full.csv', 'w', newline='') as csvfile:
        ...
        table.writerow(["name", "text", "duration", "Median pitch", "Mean pitch", "Number of pulses", "Fraction of locally unvoiced frames",
                       "Jitter (local)", "Shimmer (local)", "Mean harmonics-to-noise ratio", "Pitch SD", "Intensity SD"])
        for filename in os.listdir(r._config['to-csv']):
            ...
            if os.path.isfile(f) and f.find("full_report") != -1:
                ...
                table.writerow([f_name, report_dict["full_text"], report_dict["info"]["duration"], report_dict["info"]["Median pitch"], report_dict["info"]["Mean pitch"], report_dict["info"]["Number of pulses"],
                                report_dict["info"]["Fraction of locally unvoiced frames"], report_dict["info"][
                                    "Jitter (local)"], report_dict["info"]["Shimmer (local)"], report_dict["info"]["Mean harmonics-to-noise ratio"],
